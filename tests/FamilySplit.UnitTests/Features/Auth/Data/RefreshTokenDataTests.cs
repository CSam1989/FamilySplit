...
namespace FamilySplit.UnitTests.Features.Auth.Data;

public class RefreshTokenDataTests : IDisposable
{
    private readonly AppDbContext _db;
    private readonly RefreshTokenData _sut;
    private readonly IConfiguration _config;

    public RefreshTokenDataTests()
    {
        ...
        _sut = new RefreshTokenData(_db, _config, NullLogger<RefreshTokenData>.Instance);
    }
    ...
    {
        var sut = new RefreshTokenData(_db, _config, NullLogger<RefreshTokenData>.Instance);
    ...
    public void TokenLifetime_MissingConfig_DefaultsTo30Days()
    {
        ...
        var sut = new RefreshTokenData(_db, config, NullLogger<RefreshTokenData>.Instance);
    ...
    public async Task RotateAsync_EmptySecret_ReturnsRejected()
    {
        ...
        result.Should().Be(RefreshTokenData.RotateResult.RejectedInstance);
    }
    ...
    public async Task RotateAsync_ValidToken_RotatesAndReturnsSuccess()
    {
        ...
        result.Should().BeOfType<RefreshTokenData.RotateResult.Success>();
        var success = (RefreshTokenData.RotateResult.Success)result;
    ...
    public async Task RotateAsync_RevokedWithActiveReplacement_ReturnsConcurrentRetry()
    {
        ...
        var result = await _sut.RotateAsync(issued.Secret, null, null, ct);
        result.Should().Be(RefreshTokenData.RotateResult.ConcurrentRetryInstance);
    }
    ...
    public async Task RotateAsync_WithinReuseWindow_ReturnsReused()
    {
        ...
        result.Should().BeOfType<RefreshTokenData.RotateResult.Reused>();
        var reused = (RefreshTokenData.RotateResult.Reused)result;
