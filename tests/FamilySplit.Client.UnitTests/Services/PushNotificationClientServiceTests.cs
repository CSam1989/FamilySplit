...
public class PushNotificationClientServiceTests
{
    ...
    {
        // Arrange — BrowserSubscription is now internal so the test project can reference it
        _jsMock.Setup(x => x.InvokeAsync<bool>("FamilySplitPush.requestPermission", It.IsAny<object[]>()))
            .ReturnsAsync(true);

        _pushClientMock.Setup(x => x.GetVapidPublicKeyAsync())
            .ReturnsAsync(new VapidPublicKeyResponse("test-vapid-key"));

        _jsMock.Setup(x => x.InvokeAsync<PushNotificationClientService.BrowserSubscription?>(
                "FamilySplitPush.subscribe", It.IsAny<object[]>()))
            .ReturnsAsync(new PushNotificationClientService.BrowserSubscription(
                "https://push.example.com/endpoint",
                "p256dh-key",
                "auth-secret"));

        _pushClientMock.Setup(x => x.SubscribeAsync(It.IsAny<PushSubscribeRequest>()))
        ...
        // Assert
        result.Should().BeTrue();
        _pushClientMock.Verify(
            x => x.SubscribeAsync(It.Is<PushSubscribeRequest>(r =>
                r.Endpoint == "https://push.example.com/endpoint" &&
                r.P256dh == "p256dh-key" &&
                r.Auth == "auth-secret")),
            Times.Once);
    }
