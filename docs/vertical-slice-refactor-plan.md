...
- **Format gate:** running `dotnet format FamilySplit.slnx --verify-no-changes` cold surfaced two real, pre-existing violations — not something this phase introduced, but blocking the `format-check` job that's already wired into `ci.yml`'s `ci-gate`: (1) two Notifications test files from Phase 11 (`SubscribeCommandValidatorTests.cs`, `UnsubscribeCommandHandlerTests.cs`) had CRLF line endings against the repo's `end_of_line = lf` `.editorconfig` rule; (2) the pre-existing migration `20260611090354_AddSettlementUniqueIndexAndActiveEmailFilter.cs` (predates this refactor entirely) had a UTF-8 BOM and a block-scoped namespace, violating `charset = utf-8` and the `IDE0161` file-scoped-namespace analyzer. Ran `dotnet format FamilySplit.slnx --no-restore` (apply mode) to fix both mechanically — no behavioral change, confirmed by a full rebuild + unit/client test re-run (677/422 green, unchanged). Also added a root `.gitattributes` (`* text=auto eol=lf`) since nothing at the git level was enforcing `.editorconfig`'s `end_of_line = lf` — without it, `core.autocrlf=true` on a Windows checkout can silently reintroduce CRLF on any newly-created file and fail this exact gate again.
