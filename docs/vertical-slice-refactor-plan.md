...
### ✅ Phase 13 — Delete `FamilySplit.Application` (DONE)
...
**Completion notes:** `src/FamilySplit.Application/` deleted in full via `git rm -r` — by this point it held only `DependencyInjection.cs` (an empty `AddFamilySplitApplication()` with migration-tombstone comments per slice) and its `.csproj`; every actual service had already been dissolved into a slice across Phases 3–12. Removed: the `<Project Path=.../>` entry from `FamilySplit.slnx`; the `COPY [...]` line from `src/FamilySplit.Api/Dockerfile`'s restore-layer block; the `ProjectReference` to it from `FamilySplit.Api.csproj`, `tests/FamilySplit.UnitTests/FamilySplit.UnitTests.csproj`, and `tests/FamilySplit.IntegrationTests/FamilySplit.IntegrationTests.csproj` (IntegrationTests had no source-level usage, only the stale csproj reference — confirmed via grep before removing); the `using FamilySplit.Application;` + `builder.Services.AddFamilySplitApplication();` line from `Program.cs`; and the `using FamilySplit.Application;` + the now-meaningless `AddFamilySplitApplication_RegistersAllExpectedServices` test method (it asserted against a method that no longer exists) from `DependencyInjectionTests.cs`. Also updated **CLAUDE.md**'s solution-structure tree (dropped the `FamilySplit.Application` line) and its "Architecture in transition"/Vertical-Slice-Architecture blockquotes, which both explicitly claimed the legacy layer was "the only place exempt" from the architecture rules — now factually wrong since the project doesn't exist — rewritten to say it's deleted. Left the large "## Application Layer (FamilySplit.Application)" reference section in CLAUDE.md in place but retitled "— DELETED (historical reference only)" with an updated warning banner, since it documents the *old* method names/shapes as a map to their new slice homes; full removal of that section is explicitly Phase 14's docs-sweep job, not this phase's — didn't scope-creep into it. **Verification:** Release build 0 warnings/0 errors (full `FamilySplit.slnx`, confirms zero dangling references to the deleted project); **677 unit** tests green (678 − 1, the removed DI test — no other test referenced `FamilySplit.Application`); **422 client** tests green (unaffected). Integration tests build clean (`FamilySplit.Application` reference removed with no source fallout).

### 🟡 Phase 14 — Hardening, docs, CI (mostly done — E2E pass + push are pending)
- [x] Integration smoke tests for the seams: `POST /auth/refresh` returns non-500 (proves composed "auth" rate-limit policy exists); `POST /hubs/notifications/negotiate` returns 401-not-404 (proves module-mapped hub); dev OpenAPI document lists module endpoints.
- [x] `dotnet format FamilySplit.slnx --verify-no-changes` (CI format gate).
- [x] Update **CLAUDE.md** (solution structure, conventions, validation/logging sections all reference the old layering; document the CQRS command/query rules and response semantics) + `.github/copilot-instructions.md`.
- [ ] Full E2E pass locally (publish client wwwroot per CI recipe) — the flows exercise every re-query path through the real UI. **Blocked in this environment:** Docker/Testcontainers is unavailable here, so `FamilySplit.E2ETests` (and `FamilySplit.IntegrationTests`) can only be confirmed to *compile* locally — they run for real in CI (`.github/workflows/ci.yml`'s `e2e-tests`/`integration-tests` jobs, which provision Postgres via Testcontainers on `ubuntu-latest`).
- [ ] Push branch; full CI green before merging (deploy auto-fires on CI success on `main`). **Not done** — pushing requires explicit user confirmation per this session's operating rules; the branch is fully committed and ready to push on request.

**Completion notes:**
- **Seam smoke tests:** added `tests/FamilySplit.IntegrationTests/SeamSmokeTests.cs` (3 tests): `AuthRefresh_WithoutCookie_ReturnsUnauthorized_NotServerError` (anonymous `POST /auth/refresh` → 401, proving `AuthModule`'s own `AddRateLimiter` call for the `"auth"` policy composes with the host's rather than replacing it — a missing policy would 500 at `.RequireRateLimiting("auth")`, not 401); `NotificationsHubNegotiate_WithoutAuth_ReturnsUnauthorized_NotNotFound` (anonymous `POST /hubs/notifications/negotiate` → 401, proving `NotificationsModule` actually maps `NotificationHub` — an unmapped hub would 404); `DevOpenApiDocument_ListsEndpointsFromMultipleFeatureModules` (forces `UseEnvironment("Development")` via `Factory.WithWebHostBuilder`, fetches `/openapi/v1.json`, asserts paths from 5 different modules — Users, Dashboard, Groups, Notifications, Auth — proving the explicit `IFeatureModule[] modules` array in `Program.cs` actually wired every module's `MapEndpoints`). All three compile clean; execution is CI-only per the Docker constraint above.
- **Format gate:** running `dotnet format FamilySplit.slnx --verify-no-changes` cold surfaced two categories of violation, not something this phase introduced: (1) two Notifications test files from Phase 11 (`SubscribeCommandValidatorTests.cs`, `UnsubscribeCommandHandlerTests.cs`) were checked out as CRLF against the repo's `end_of_line = lf` `.editorconfig` rule — turned out to be a **local, Windows-checkout-only** artifact of `core.autocrlf=true` (the git-stored blob was already LF, which is why `git diff`/`git status` showed no real change once reformatted — this specific issue likely never would have failed CI's Linux runners, but it did fail local verification, which is the whole point of running this gate proactively); (2) the pre-existing migration `20260611090354_AddSettlementUniqueIndexAndActiveEmailFilter.cs` (predates this refactor entirely) had a genuine stored UTF-8 BOM and a block-scoped namespace, violating `charset = utf-8` and the `IDE0161` file-scoped-namespace analyzer — this one **would** fail CI's `format-check` job (already wired into `ci.yml`'s `ci-gate`) regardless of OS, since it's a real content difference, not a checkout artifact. Ran `dotnet format FamilySplit.slnx --no-restore` (apply mode) to fix both mechanically — no behavioral change, confirmed by a full rebuild + unit/client test re-run (677/422 green, unchanged). Also added a root `.gitattributes` (`* text=auto eol=lf`) so a genuine CRLF file can never actually get committed in the future, regardless of a contributor's local `core.autocrlf` setting.
- **Docs sweep (CLAUDE.md):** deleted the entire "Application Layer (FamilySplit.Application)" reference section (it described `AdminService`/`FamilyService`/`GroupService`/`ExpenseService`/`SettlementService`/`ActivityService`/`ParticipantSeeder`/`BalanceCalculator`/`SettlementOptimiser`/`SplitCalculator` — all deleted across Phases 3–12); salvaged the still-relevant EF-Core-10 navigation-property cycle-detection gotcha into the Vertical Slice Architecture section (query handlers hit this same issue). Rewrote the "API Layer" section — its old "Endpoint Groups" table listed nine `Api/Endpoints/*.cs` files that no longer exist; replaced with a "Feature modules (route prefixes)" table and fixed the stale `AdminService.RequireGlobalAdminAsync()` authorization reference to point at `IAdminData.IsGlobalAdminAsync`/`AdminGate`. Rewrote "Validation Standards → Backend" — its file/validator table (`ActivityValidator.cs`, `AdminValidator.cs`, etc.) named files that don't exist; replaced with the actual `{UseCase}CommandValidator` convention and per-module `AddValidatorsFromAssembly` registration. Fixed two smaller stale references: `WeightCalculator`'s doc comment said "Application layer" (now `FamilySplit.Common`), and a code comment path said `Application/Core/WeightCalculator.cs` (now `FamilySplit.Common/Calculations/WeightCalculator.cs`).
- **Docs sweep (`.github/copilot-instructions.md`):** fixed the "Migrated so far: Expenses, Dashboard" line (now lists all ten slices + notes `FamilySplit.Application` is deleted) and replaced the closing "Caller identity" code sample — it showed a raw `_db.FamilyMembers` query, which is exactly the pattern the ADR-001 rules earlier in the *same file* forbid in a command handler — with the actual `IGroupMembershipGuard.GetCallerFamilyIdAsync` / per-slice `I{Slice}Data` seam.
- **Verification:** Release build 0 warnings/0 errors (full `FamilySplit.slnx`); **677 unit** / **422 client** tests green (unchanged by the format-only fixes); `IntegrationTests` and `E2ETests` build clean including the new smoke tests; `dotnet format FamilySplit.slnx --verify-no-changes` now passes cold.
