...
public static class SettlementEndpoints
{
    public static WebApplication MapSettlementEndpoints(this WebApplication app)
    {
        ...
        grp.MapGet("/", async (Guid groupId, Guid activityId, SettlementService svc, HttpContext ctx) =>
        {
            var callerId = ctx.User.GetUserId();
            var settlements = await svc.ListAsync(activityId, callerId);
        ...
        grp.MapPost("/", async (Guid groupId, Guid activityId, SettlementService svc, HttpContext ctx) =>
        {
            var callerId = ctx.User.GetUserId();
            var settlements = await svc.GenerateAsync(activityId, callerId);
            return Results.Ok(settlements);
        });
        ...
        grp.MapGet("/{settlementId:guid}", async (Guid groupId, Guid activityId, Guid settlementId,
            SettlementService svc, HttpContext ctx) =>
        {
            var callerId = ctx.User.GetUserId();
            var detail = await svc.GetDetailAsync(settlementId, callerId);
        ...
        grp.MapPost("/{settlementId:guid}/confirm-sent", async (Guid groupId, Guid activityId,
            Guid settlementId, SettlementService svc, HttpContext ctx) =>
        {
            var callerId = ctx.User.GetUserId();
            var detail = await svc.ConfirmSentAsync(settlementId, callerId);
            return Results.Ok(detail);
        });
        ...
        {
            var callerId = ctx.User.GetUserId();
            var detail = await svc.ConfirmReceivedAsync(settlementId, callerId);
            return Results.Ok(detail);
        });
        ...
        {
            var callerId = ctx.User.GetUserId();
            var balances = await svc.GetBalancesAsync(activityId, callerId);
