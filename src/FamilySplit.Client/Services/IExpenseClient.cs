...
public interface IExpenseClient
{
    ...
    Task<CreatedResponse> CreateAsync(Guid groupId, Guid activityId, [Body] CreateExpenseRequest request);

    [Put("/groups/{groupId}/activities/{activityId}/expenses/{expenseId}")]
    Task UpdateAsync(Guid groupId, Guid activityId, Guid expenseId, [Body] UpdateExpenseRequest request);
