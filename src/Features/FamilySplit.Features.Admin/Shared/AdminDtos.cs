...
// ── Response DTOs ─────────────────────────────────────────────────────────────
// The Admin slice owns its copies of these (identical JSON property names to the legacy
// Families DTOs — the Admin query integration tests lock the wire format). Families gets its
// own copies when it migrates (Phase 10); the duplication is by design (per-slice DTOs).
